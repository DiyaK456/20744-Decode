...
public class MotorSpeedHolderTest extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        dpadDown = new ButtonBlock().onTrue(() -> {targetSpeed/=2;setVelocity(targetSpeed, AngleUnit.DEGREES);});
        dpadUp = new ButtonBlock().onTrue(() -> {targetSpeed*=2;setVelocity(targetSpeed, AngleUnit.DEGREES);});
        waitForStart();
        if (opModeIsActive()) {
